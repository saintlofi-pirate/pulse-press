    ...
    public function capture(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        if (!$this->postIsPublic($postId)) {
            return new WP_Error(
                ...
                __('That post is not available for capturing right now.', 'pulsepress'),
                ['status' => 404]
            );
        }
        ...
        if (!Reactions::isValid($reactionType)) {
            return new WP_Error(
                ...
                __('That reaction type is not available on this site.', 'pulsepress'),
                ['status' => 422]
            );
        }
        ...
        if ($consent !== true) {
            return new WP_Error(
                ...
                __('You must agree to the consent statement before submitting your email.', 'pulsepress'),
                ['status' => 422]
            );
        }
        ...
        if (!Captures::isValidSource($source)) {
            return new WP_Error(
                ...
                __('That capture source is not allowed.', 'pulsepress'),
                ['status' => 422]
            );
        }
        ...
        if ($email === '' || strlen($email) > Captures::EMAIL_MAX_LENGTH || !is_email($email)) {
            return new WP_Error(
                ...
                __('Please enter a valid email address.', 'pulsepress'),
                ['status' => 422]
            );
        }
        ...
        if ($record->status === CaptureRecord::STATUS_ALREADY_EXISTS) {
            return new WP_Error(
                ...
                __('We already have your email saved for this post.', 'pulsepress'),
                ['status' => 409]
            );
        }
