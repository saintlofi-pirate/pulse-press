# PulsePress Pro Roadmap

This doc holds everything the v1 plan deferred about Pro: the product spec, the codebase plan, and the per-feature designs. It does *not* replace `docs/pulsepress-v1-plan.md §Free vs Pro Scope` (the durable Free/Pro principle) or `docs/pulsepress-v1-plan.md §Pro Extension Seams` (the hook contract). Read this when:
...
- Stored as a WP option: `pulsepress_pro_license` (string), `pulsepress_pro_license_status` (`active|invalid|expired|grace`), `pulsepress_pro_license_checked_at` (unix).
...
Self-hosted on Anthropic-stack-equivalent (whatever lives at `pulsepress.io` when we get there). Stack:
...
Separate plugin folder (`pulsepress-pro/`), separate Git repo (`pulsepress-pro` private). **Not** a monorepo with Free — keeps each repo's `composer.json` and Vite config independent and makes the license boundary obvious (Free is GPL; Pro is GPL-with-paid-distribution, same as Yoast / EDD).

Repo layout mirrors Free:

```
pulsepress-pro/
├── pulsepress-pro.php       # plugin entry, declares "Requires Plugins: pulse-press"
├── app/
...
├── tests/Pest.php
└── composer.json            # PSR-4: PulsePressPro\
```

Namespace: `PulsePressPro\` → `app/`.

### 2.2 Bootstrap & compatibility

Pro plugin's entry:

1. `Requires Plugins: pulse-press` header (WP 6.5+ enforces this; Pro stays deactivated if Free isn't active).
2. Checks `defined('PULSEPRESS_VERSION')` and `version_compare(PULSEPRESS_VERSION, '1.0.0', '>=')`. If not, admin notice + don't boot.
...
- Pro NEVER imports a class from Free's `app/` directly. The only Free types Pro depends on are explicit public types (likely `\PulsePress\Analytics\AggregationResult`, `\PulsePress\Settings\Settings`) — and only via interfaces that Free will publish as a "stable surface" once Pro starts shipping.
...
- Pro's own tables: `wp_pulsepress_pro_esp_jobs`, `wp_pulsepress_pro_ab_assignments`, `wp_pulsepress_pro_weekly_agg`, `wp_pulsepress_pro_monthly_agg`. Migrations live in Pro's own `Schema::VERSION`.
- Pro registers its REST routes under `pulsepress/v1/pro/*`. Each route has its own permission callback that consults license status before serving anything beyond a stub response.

### 2.4 Build pipeline

- Pro uses its own Vite config; emits its own admin + widget chunks loaded via Pro's own `wp_enqueue_script` calls.
- Pro's admin JS calls `window.PulsePressAdmin.registerTabRenderer / registerCardRenderer / registerPanelRenderer` (shipped in Session 9.6) to mount into Free's admin SPA. No Preact-from-Free import — Pro can ship its own Preact instance if needed (file size budget allows).
- Pro's widget JS attaches via the `pulsepress_widget_data` filter (PHP, Session 3) and a separate widget enqueue tied to A/B variant assignment.
...
- Hook into `pulsepress_after_capture` (action, Session 4). On every captured email, push a row into `wp_pulsepress_pro_esp_jobs` with status `queued`.
...
- Surface in Free admin: nothing extra. ESP-related UI lives entirely in Pro's tab, registered via `pulsepress_admin_tabs`.
...
- Test config stored in `wp_pulsepress_pro_ab_tests`: `id, name, variant_a (json), variant_b (json), traffic_split (int %), starts_at, ends_at, status`.
- Variant assignment recorded in `wp_pulsepress_pro_ab_assignments` on first reaction; subsequent events for the same visitor read the assignment.
- Widget receives variant id via `pulsepress_widget_data` filter; the widget loads variant-specific settings before paint (no flash of original variant).
...
- Implemented as a server-side query layer wrapping Free's `AnalyticsRepository`. Pro doesn't touch Free's repo; it registers a Pro-only REST route (`/pulsepress/v1/pro/analytics`) that runs the same queries with extra WHERE clauses (category id IN, author id IN, post_date BETWEEN, user_id IS NOT NULL).
- Segment definitions persist in `wp_pulsepress_pro_segments` as JSON (post_type, taxonomy, author, post_date range, user_logged_in). Reusable; admin saves them.
- Admin SPA wiring: Pro registers a "Segments" sidebar in the Analytics tab via `pulsepress_admin_analytics_panels`. Selecting a segment refetches via the Pro endpoint instead of Free's.
...
- New tables `wp_pulsepress_pro_weekly_agg` (week starts Monday UTC, configurable via filter) and `wp_pulsepress_pro_monthly_agg`.
- Pro hooks `pulsepress_after_aggregate` (action, Session 8). After Free's daily aggregator finishes, Pro upserts the affected week and month rows.
- Comparison metric cards (already documented in Free as injection target via `pulsepress_admin_metric_cards`): Pro registers cards like "vs previous 30 days" and feeds them with two summed windows from the rollup tables.
- `pulsepress_analytics_max_days` filter raised to ~30 years in Pro (effectively unlimited). The performance budget is preserved because Pro queries the rollup, not `daily_agg`.

### 3.5 White-Label

**Job**: agencies hide "PulsePress" attribution and apply their own brand.

**Design**:

- Three settings (Pro-only, in Pro tab):
  - **Brand name** (string, default "PulsePress"). Used in the admin page title, dashboard widget, README link.
...
- White-label settings are read by Pro and used to filter Free's `pulsepress_widget_data` and the admin SPA's `pulsepress_admin_data` payloads.
...
- Each webhook row: `url`, `secret` (used for HMAC signature header `X-PulsePress-Signature`), `events` (json array), `active` (bool), `last_status`, `last_attempted_at`.
...
- Pro REST route `POST /pulsepress/v1/pro/reports` accepts a window spec; returns a report id immediately.
- A scheduled job runs the heavy query in chunks, writes to a temp table, then writes a CSV to a temporary uploads-dir path. Status surfaces via `GET /pulsepress/v1/pro/reports/{id}` (queued / running / ready / failed).
...
- Register routes under `pulsepress/v1/pro/*`.
- Register settings via `pulsepress_settings` / `pulsepress_settings_default`.
- Register admin UI surface via `pulsepress_admin_tabs`, `pulsepress_admin_metric_cards`, `pulsepress_admin_analytics_panels`, and the JS `window.PulsePressAdmin.register*Renderer` API.
- Modify analytics behaviour via `pulsepress_analytics_window`, `pulsepress_analytics_max_days`.
- Hook side effects via `pulsepress_after_react`, `pulsepress_after_capture`, `pulsepress_after_aggregate`, `pulsepress_settings_saved`.
- Modify capture handling via `pulsepress_capture_sources`, `pulsepress_capture_email`, `pulsepress_consent_text_version`.
- Modify CSV via `pulsepress_export_columns`.
- Modify widget bootstrap via `pulsepress_widget_data`, `pulsepress_widget_icons`, `pulsepress_widget_container_attrs`, `pulsepress_visibility_mode`.
...
- **Should `pulsepress_after_capture` be sync or async?** Currently sync; ESP push is fire-and-forget into the queue. If a user wants synchronous double-opt-in confirmation pages, the action fires synchronously and Pro must respond fast (< 200 ms). Probably fine; flag for benchmarking.
