...
The recently expanded `docs/pulsepress-v1-plan.md §Pro Extension Seams` lists the hooks Pro is expected to attach through, but half of them have no implementation on Free's side. `pulsepress_admin_tabs`, `pulsepress_admin_metric_cards`, `pulsepress_admin_analytics_panels` exist only as filter documentation; the SPA does not yet read them. Without this slice, a Pro plugin can register tabs and panels via PHP filter and *nothing happens* — the SPA hardcodes its tab list and panel layout. The injection surface needs to be real before Pro starts shipping.

Free remains generous: this slice only adds extension points, the existing UI keeps rendering identically when no Pro extension is registered. Privacy stays first-class: Pro-registered REST routes are gated independently; Free does not relay Pro data on the front-end widget. Hooks-first: the PHP filter side ships now; the JS renderer registry (`window.PulsePressAdmin.registerTabRenderer`, `…CardRenderer`, `…PanelRenderer`) becomes the documented JS contract Pro mounts components against.

## What Changes

- `AdminServiceProvider::maybeEnqueueAssets` extends the `PulsePressAdminData` payload with:
  - `tabs: list<{id, label, order}>` — Free's five tabs (display 10, analytics 20, reactions 30, capture 40, privacy 50) merged with anything returned by `apply_filters('pulsepress_admin_tabs', $tabs)`. Sorted ascending by `order` before emission.
  - `metricCards: list<{id, title, value, helper, emphasis, renderJs?}>` — from `apply_filters('pulsepress_admin_metric_cards', [])`. Free does not consume the filter itself; Pro fills it.
  - `analyticsPanels: list<{id, title, helper, data?, renderJs?}>` — from `apply_filters('pulsepress_admin_analytics_panels', [])`.
- `resources/admin/index.tsx` exposes `window.PulsePressAdmin` with three registration methods:
...
- A `pulsepress:extension-registered` `CustomEvent` fires on `window` after each registration so the SPA re-renders if Pro registers after Free has already mounted.
...
- Add CSS scaffolding for `.pulsepress-extension-mount` (matches the surrounding visual rhythm) and `.pulsepress-extension-fallback`.
...
- `pro-injection-surface`: defines the PHP filters (`pulsepress_admin_tabs`, `pulsepress_admin_metric_cards`, `pulsepress_admin_analytics_panels`), the corresponding JS extension contract (`window.PulsePressAdmin.register*Renderer`), the SPA loading flow, and the fallback UX for partial Pro activation.
...
- **Modified files**: `app/Providers/AdminServiceProvider.php` (payload extension), `resources/admin/types.ts` (new payload fields + `PulsePressAdmin` global), `resources/admin/index.tsx` (registry exposure), `resources/admin/App.tsx` (dynamic tabs), `resources/admin/sections/AnalyticsSection.tsx` (extra cards + panels), `resources/admin/styles/admin.css` (extension styling), `docs/hooks-and-filters.md` (promote `pulsepress_admin_tabs` and `pulsepress_admin_metric_cards` from "planned" to "shipped"; reinforce `pulsepress_admin_analytics_panels` row).
- **REST API**: unchanged. Pro registers its own routes independently under `pulsepress/v1/pro/*`.
- **Database**: unchanged.
- **Filters introduced**: `pulsepress_admin_tabs`. Promoted to shipped: `pulsepress_admin_metric_cards`, `pulsepress_admin_analytics_panels` (already documented; SPA consumes them now).
- **JS contract introduced**: `window.PulsePressAdmin` exposing `registerTabRenderer / registerCardRenderer / registerPanelRenderer`. The `pulsepress:extension-registered` `CustomEvent` lets late registrations trigger a re-render.
