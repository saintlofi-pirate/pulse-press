...
`AdminServiceProvider` SHALL emit an admin payload `tabs` array. The array SHALL be assembled by merging Free's five built-in tabs (`display`, `analytics`, `reactions`, `capture`, `privacy`, each with a numeric `order`) with whatever `apply_filters('moonfarmer_reactions_lead_capture_admin_tabs', [])` returns. Entries SHALL be deduplicated by `id` (Free wins on conflict). The merged array SHALL be sorted ascending by `order`, ties broken by `id` alphabetical.

#### Scenario: Free tabs render unchanged when no extension registered
- **GIVEN** no plugin attaches to `moonfarmer_reactions_lead_capture_admin_tabs`
...
- **GIVEN** a plugin returns `[['id' => 'esp', 'label' => 'ESP', 'order' => 25]]` from `moonfarmer_reactions_lead_capture_admin_tabs`
...
`AdminServiceProvider` SHALL emit `metricCards` in the admin payload as the result of `apply_filters('moonfarmer_reactions_lead_capture_admin_metric_cards', [])`. Free SHALL NOT append its own cards to this array — Free's four built-in cards render via the SPA's static component composition. Pro-supplied entries SHALL be serialised as-is.

#### Scenario: No Pro cards by default
- **GIVEN** no plugin attaches to `moonfarmer_reactions_lead_capture_admin_metric_cards`
- **WHEN** the admin payload is built
...
`AdminServiceProvider` SHALL emit `analyticsPanels` in the admin payload as the result of `apply_filters('moonfarmer_reactions_lead_capture_admin_analytics_panels', [])`. Order SHALL preserve filter return order.

#### Scenario: Default empty
- **GIVEN** no plugin attaches to `moonfarmer_reactions_lead_capture_admin_analytics_panels`
- **WHEN** the admin payload is built
- **THEN** `analyticsPanels` SHALL be an empty array

### Requirement: JS renderer registry exposed on window

The admin SPA SHALL expose a `window.MoonfarmerReactionsLeadCaptureAdmin` object with three registration methods: `registerTabRenderer(id, fn)`, `registerCardRenderer(id, fn)`, `registerPanelRenderer(id, fn)`. The registry SHALL be created before the SPA mounts so Pro scripts loaded after Free's bundle can call the methods synchronously.

#### Scenario: Pro registers a tab renderer
- **GIVEN** Free's admin bundle has loaded
- **WHEN** Pro calls `window.MoonfarmerReactionsLeadCaptureAdmin.registerTabRenderer('esp', fn)`
...
- **WHEN** Pro calls `window.MoonfarmerReactionsLeadCaptureAdmin.registerTabRenderer('esp', fn)` after mount
- **THEN** the `moonfarmer-reactions-lead-capture:extension-registered` `CustomEvent` SHALL fire on `window` and the `ExtensionMount` SHALL re-render with Pro's renderer
...
- **GIVEN** Pro registers a tab via `moonfarmer_reactions_lead_capture_admin_tabs` but its bundle fails to load
