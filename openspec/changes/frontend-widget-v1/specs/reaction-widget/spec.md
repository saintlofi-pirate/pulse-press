...
`Moonfarmer\ReactionsLeadCapture\Providers\WidgetServiceProvider` SHALL register a `wp_enqueue_scripts` callback that enqueues the widget's JS and CSS only when `is_admin() === false` and either `is_singular('post') === true` or `apply_filters('moonfarmer_reactions_lead_capture_widget_enqueue', false)` returns `true`. On every other request, no widget asset SHALL be added to the page.

#### Scenario: Front-end single post

- **WHEN** a visitor loads a published single post
- **THEN** `dist/js/widget.<hash>.js` and `dist/assets/widget.<hash>.css` are enqueued and `window.MoonfarmerReactionsLeadCaptureData` is emitted

#### Scenario: Front-end archive page

- **WHEN** a visitor loads the home page or a category archive without the `moonfarmer_reactions_lead_capture_widget_enqueue` filter overridden
...
- **WHEN** a plugin registers `add_filter('moonfarmer_reactions_lead_capture_widget_enqueue', '__return_true')` and a visitor loads a page or a custom post type
...
`Moonfarmer\ReactionsLeadCapture\View\Manifest::resolve(string $entry): array{js: ?string, css: ?string}` SHALL read `dist/.vite/manifest.json` and return the hashed URLs for the given source entry. Results SHALL be cached in a transient keyed `moonfarmer_reactions_lead_capture_vite_manifest_v1` for 24 hours; the cache key SHALL incorporate the manifest file's `filemtime()` so a fresh build invalidates the cache automatically.
...
The Preact widget SHALL render exactly six `<button>` elements (one per reaction in `MoonfarmerReactionsLeadCaptureData.reactions`), each containing an inline SVG icon, a visible count, an `aria-label` naming the reaction, and `aria-pressed` reflecting whether that reaction is the visitor's current choice. On first paint, counts SHALL come from a `GET /counts/{postId}` request; the active reaction SHALL come from `localStorage` under `moonfarmer-reactions-lead-capture:reaction:{postId}`.
...
- **THEN** the love button shows `1`, `aria-pressed="true"`, `localStorage['moonfarmer-reactions-lead-capture:reaction:42']` is `'love'`, and no error message renders
...
- **THEN** the love count returns to its pre-click value, `aria-pressed` returns to `false`, `localStorage['moonfarmer-reactions-lead-capture:reaction:42']` is removed (or restored to its previous value), and an inline "Please try again." message renders for 4 seconds
...
The plugin SHALL register `GET /wp-json/moonfarmer-reactions-lead-capture/v1/counts/(?P<post_id>\\d+)` as a public endpoint (`permission_callback: __return_true`). It SHALL return `{post_id, counts, cached}` where `counts` is always a JSON object (never an array literal, even when empty). The endpoint SHALL serve from a `moonfarmer_reactions_lead_capture_counts_{post_id}` transient when present; on miss it SHALL execute a single grouped SELECT against `<prefix>moonfarmer_reactions_lead_capture_reactions` and write the result into the transient with a 300-second TTL before returning.
