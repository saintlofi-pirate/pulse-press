...
- Admin opens `/wp-admin/options-general.php?page=pulsepress` and sees a Preact SPA that loads settings, renders the Display tab by default, and shows a live widget preview alongside the controls.
...
**Alternative considered**: HTML5 history API. Rejected — would require server-side handling of arbitrary `/pulsepress/<tab>` URLs, conflicts with WP's admin routing, and offers no real UX win over hash routing for this scale.
...
`LivePreview.tsx` imports `ReactionBar` from `resources/widget/components/ReactionBar.tsx`. It builds a `PulsePressData` object from the in-flight admin settings + mock counts and renders the bar inside a labelled card. Mock counts: `{love: 24, insightful: 12, funny: 6, sad: 3, surprised: 2, angry: 1}` so every reaction has a visible number.
...
Each setting group is a `<section class="pulsepress-admin-card">` with a header (`<h2>` with sentence case), a one-line helper paragraph, and the controls. Cards stack vertically; spacing is `1.25rem` between cards. The live preview is a fixed-position panel on screens wider than ~960 px; on narrower screens it stacks below the active section.
...
### D9. Localized payload `PulsePressAdminData`

```js
window.PulsePressAdminData = {
    restRoot: 'https://example.com/wp-json/pulsepress/v1/',
...
Passes through `apply_filters('pulsepress_admin_data', $payload)` before emission. Pro / extensions can add fields.
...
Enqueue gates: `add_action('admin_enqueue_scripts', fn($hook) => $hook === 'settings_page_pulsepress' && enqueue())`.

## Risks / Trade-offs

- **Risk**: the admin bundle bloats over time as Pro adds tabs. → Mitigation: filterable `pulsepress_admin_data` lets Pro inject lazy-load triggers; bundle size for Free stays bounded. We'll add a CI check in Session 11.
...
3. Visit `/wp-admin/options-general.php?page=pulsepress` as admin.
...
- **Q1**: should the admin show the current PulsePress version near the wordmark? → **Decided yes** — keeps "what version am I on?" question one glance away.
