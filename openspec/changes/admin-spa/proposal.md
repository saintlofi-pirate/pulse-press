...
Session 6a put the settings REST contract in place. This slice gives admins the page they'll actually use to drive Moonfarmer Reactions Lead Capture: a Preact-based single-page admin that feels like a modern product, not a wp-admin form. The settings page is where the rubber meets the road for the whole "modern, sleek, minimal, clean, smooth SPA with live widget preview" design bar codified in `docs/moonfarmer-reactions-lead-capture-v1-plan.md` §Admin UI Design Direction.
...
- Add a second Vite entry `resources/admin/index.tsx` that mounts a Preact app into `#moonfarmer-reactions-lead-capture-admin` (the placeholder Session 6a registered). The admin app is a separate bundle from the widget bundle so the front-end JS budget stays unaffected.
...
- Add `resources/admin/api.ts` with `fetchSettings()` and `saveSettings(partial)` typed wrappers around the Session 6a REST contract. The save call sends `X-WP-Nonce` from `MoonfarmerReactionsLeadCaptureAdminData.nonce`.
...
- Add a layout `resources/admin/components/Moonfarmer Reactions Lead CaptureLayout.tsx` (header with Moonfarmer Reactions Lead Capture wordmark + version + status pill; left/top tabs; main pane; live preview pane).
...
- Add `resources/admin/components/LivePreview.tsx` — wraps the existing `ReactionBar` from the widget source tree, feeds it the in-flight settings as if MoonfarmerReactionsLeadCaptureData reflected them, supplies mock counts (`{love: 24, insightful: 12, funny: 6, sad: 3, surprised: 2, angry: 1}`) clearly labelled "Preview", mutes clicks (handler is a no-op + announces "Preview is read-only" via aria-live), and re-renders on every settings change.
...
- Add `resources/admin/styles/admin.css` — scoped under `.moonfarmer-reactions-lead-capture-admin` with CSS custom properties matching the widget's accent. Card-based layout; generous spacing; system font; sentence case; 150–200 ms transitions; `:focus-visible` rings; `prefers-reduced-motion` handling.
- Update `vite.config.js` for dual entries (widget + admin), both emitting hashed JS + CSS under `dist/`.
- Add `Moonfarmer\ReactionsLeadCapture\Providers\AdminServiceProvider` (registered before WidgetServiceProvider in `app/bootstrap.php`) that:
  - On `admin_enqueue_scripts`, when the current page is the Moonfarmer Reactions Lead Capture settings screen, registers + enqueues the admin JS + CSS via the same `Manifest` reader from Session 3, and emits `window.MoonfarmerReactionsLeadCaptureAdminData = {restRoot, nonce, settings, defaults, choices, schemaVersion, reactions, version}` via `wp_localize_script`. The payload is filterable via `moonfarmer_reactions_lead_capture_admin_data`.
...
- **New files**: `resources/admin/index.tsx`, `resources/admin/App.tsx`, `resources/admin/api.ts`, `resources/admin/types.ts`, `resources/admin/hooks/useSettingsState.ts`, `resources/admin/components/Moonfarmer Reactions Lead CaptureLayout.tsx`, `resources/admin/components/SectionNav.tsx`, `resources/admin/components/LivePreview.tsx`, `resources/admin/components/fields/*.tsx` (6 files), `resources/admin/sections/DisplaySection.tsx`, `resources/admin/sections/ReactionsSection.tsx`, `resources/admin/sections/CaptureSection.tsx`, `resources/admin/sections/PrivacySection.tsx`, `resources/admin/styles/admin.css`, `app/Providers/AdminServiceProvider.php`, `tests/Unit/AdminServiceProviderTest.php`.
...
- **Filters introduced**: `moonfarmer_reactions_lead_capture_admin_data` (PHP, filterable admin localized payload).
...
- **Free/Pro boundary**: untouched. Pro can hook `moonfarmer_reactions_lead_capture_admin_data` to inject additional config (license key UI seed, etc.) and `moonfarmer_reactions_lead_capture_settings` to layer extra fields. The SPA dynamically renders any unknown setting fields as a generic JSON inspector for forward compatibility.
