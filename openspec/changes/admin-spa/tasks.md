...
- [x] 1.2 Create `resources/admin/index.tsx` mounting `<App />` into `#pulsepress-admin` on `DOMContentLoaded`.
- [x] 1.3 Create `resources/admin/types.ts` with `PulsePressAdminData` interface mirroring the PHP payload.
- [x] 1.4 Create `resources/admin/api.ts` with `fetchSettings()` and `saveSettings(partial)`.

## 2. State + hooks

- [x] 2.1 Create `resources/admin/hooks/useSettingsState.ts` exposing `{ settings, defaults, choices, schema, savingFields, errors, update(partial), reset(sectionKeys) }`. Initial state seeds from `PulsePressAdminData.settings`. `update` is the optimistic save flow described in design.md D4.

## 3. Layout + nav

- [x] 3.1 Create `resources/admin/App.tsx` with hash-routed tab switching and a `popstate` listener.
- [x] 3.2 Create `resources/admin/components/PulsePressLayout.tsx` (header with wordmark + version + status pill; main two-column layout where wide screens show preview alongside, narrow screens stack).
...
- [x] 7.1 Create `resources/admin/styles/admin.css` scoped under `.pulsepress-admin` with: CSS custom properties (`--pp-accent`, `--pp-text`, `--pp-bg`, etc.), header layout, tablist styling, card styling, field primitives, preview pane layout, save-state pills (`.pulsepress-pill--saved`, `.pulsepress-pill--saving`), error styling, reduced-motion handling.
...
- [x] 9.1 `tests/Unit/AdminServiceProviderTest.php`: enqueue does NOT fire on the front end; enqueue DOES fire when hook context matches the settings page; PulsePressAdminData payload contains the required fields and passes through `pulsepress_admin_data` filter.
...
- [x] 10.2 Reactivate plugin. Visit `/wp-admin/options-general.php?page=pulsepress` as admin. Confirm SPA renders with the header, tabs, Display section, and preview pane.
...
- [x] 11.2 Update `docs/hooks-and-filters.md`: add `pulsepress_admin_data` filter.
