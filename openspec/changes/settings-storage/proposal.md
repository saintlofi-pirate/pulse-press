...
Free remains generous: every setting that today defaults via filter (positive reactions, auto-insert post types, widget enqueue gating, consent text version) becomes admin-editable with sensible defaults. The hook surface stays unchanged — filters still win, so existing PHP customisations keep working. Privacy stays first-class: every option key is namespaced `pulsepress_*`, sanitisation rejects unexpected keys, and the REST endpoint requires `manage_options`.

## What Changes

- Add `PulsePress\Settings\Settings` final class holding the canonical schema: a public `const DEFAULTS` map keyed by setting name, plus per-setting sanitiser closures in `Settings::sanitise(array $input): array`. Setting keys:
...
- Add `PulsePress\Settings\SettingsRepository` with `get(): array` (merging stored option over `Settings::DEFAULTS` after passing through `apply_filters('pulsepress_settings', $settings)`) and `save(array $partial): array` (sanitises, merges over current, writes the option, fires `do_action('pulsepress_settings_saved', $newSettings, $previousSettings)`, returns the new full settings).
- Add `PulsePress\Http\Controllers\SettingsController` with `read(\WP_REST_Request $request)` (GET, returns `{settings, defaults, choices}` so the SPA knows the allowed values for every selector) and `update(\WP_REST_Request $request)` (PUT/PATCH, sanitises the body via `Settings::sanitise`, calls `$repository->save($input)`, returns the merged result).
- Add `PulsePress\Providers\SettingsServiceProvider` that registers DI bindings and registers `/wp-json/pulsepress/v1/settings` (GET + POST) on `rest_api_init` with `permission_callback => current_user_can('manage_options')` and a JSON-Schema `args` for every input field. Provider also registers `admin_menu` action to add the WordPress submenu under "Settings → PulsePress" — Session 6b will mount the SPA into the registered page, this session ships the placeholder page that simply outputs `<div id="pulsepress-admin"></div>` plus a "Loading…" notice.
- Add `pulsepress_settings_default` filter (runs once on first `get()` if no option exists) and `pulsepress_settings_saved` action (after every successful write).
- Wire existing front-end paths to read from `SettingsRepository`:
  - `pulsepress_positive_reactions` filter now returns the saved `positive_reactions` (filter still applies on top for code-level overrides).
  - `pulsepress_widget_auto_insert` filter now consults `auto_insert_post_types`.
  - `WidgetServiceProvider::enqueueAssets` reads `icon_style` and `theme_mode` and surfaces them into `PulsePressData`.
  - `ReactionController::react` permission_callback consults `allow_guest_reactions` — when false AND the request is unauthenticated, returns 401 `pulsepress_login_required`.
...
- `reaction-widget`: the localised `PulsePressData` payload gains `iconStyle` and `themeMode` derived from settings. The auto-insert decision and positive-reactions list draw from settings before the filter pass.
...
- **Modified files**: `app/Providers/RestServiceProvider.php` (react permission_callback consults guest-reactions toggle), `app/Providers/WidgetServiceProvider.php` (payload reads settings), `app/Reactions/Reactions.php` (no change to constants — the settings layer reads them), `app/Captures/Captures.php` (consentTextVersion checks settings before its default-then-filter chain), `app/bootstrap.php` (registers `SettingsServiceProvider`), `docs/hooks-and-filters.md` (adds `pulsepress_settings`, `pulsepress_settings_default`, `pulsepress_settings_saved`).
- **Option keys introduced**: `pulsepress_settings` (autoloaded). Existing `pulsepress_delete_on_uninstall` and `pulsepress_retention_days` are mirrored under the new settings array; the legacy keys keep working for backward compat and migrate automatically on first save.
- **REST endpoints**: `/wp-json/pulsepress/v1/settings` — `GET` (read) + `POST` (update). Requires `manage_options`.
- **Filters introduced**: `pulsepress_settings`, `pulsepress_settings_default`.
- **Actions introduced**: `pulsepress_settings_saved`.
...
- **Free/Pro boundary**: settings reads pass through `apply_filters('pulsepress_settings', ...)` so Pro can layer additional fields. Pro never modifies Settings::DEFAULTS directly.
