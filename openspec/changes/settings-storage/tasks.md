...
- [ ] 1.1 Create `app/Settings/Settings.php` with `final class` in namespace `PulsePress\Settings`. `public const SCHEMA_VERSION = 1`. `public const DEFAULTS` map with every setting key and default. `public const CHOICES` map for enumerable fields. Static `sanitise(array $input): array` that produces a complete sanitised settings array; unknown keys dropped; out-of-range clamped; invalid enums fall back to defaults.
...
- [ ] 2.2 `get(): array` reads `pulsepress_settings`, falls back to defaults, merges, passes through `pulsepress_settings` filter, memoises. Handles legacy keys (`pulsepress_delete_on_uninstall`, `pulsepress_retention_days`) when the new option is absent.
- [ ] 2.3 `save(array $partial): array` merges sanitised input over current, writes the option (with `_version`), updates the memo, fires `do_action('pulsepress_settings_saved', $new, $previous)`, returns the new array.
...
- [ ] 3.2 Create `app/Providers/SettingsServiceProvider.php`. Bindings in `register()`. In `boot()`:
  ...
  - Hook `pulsepress_positive_reactions` filter at priority 5 to seed from settings (lets user-registered filters at priority 10+ still override).
  - Hook `pulsepress_widget_auto_insert` at priority 5 to honor `auto_insert_post_types`.
  - Hook `pulsepress_consent_text_version` at priority 5 to read from settings.
...
- [ ] 4.1 `RestServiceProvider::boot()` — extend the react permission_callback: nonce check AND when `allow_guest_reactions` is false, also `is_user_logged_in()`. Return `WP_Error('pulsepress_login_required', '…', ['status' => 401])` instead of `false` when guest reactions are off so the response code is meaningful.
- [ ] 4.2 `WidgetServiceProvider::enqueueAssets()` — read settings and add `iconStyle`, `themeMode`, `widgetDesign`, `countVisibility`, `countThreshold` to the localised payload. Always overrideable via `pulsepress_widget_data` filter.
...
- [ ] 6.1 Reactivate plugin. `wp option get pulsepress_settings` → absent (no first-save yet).
- [ ] 6.2 `curl -k -X GET -H "X-WP-Nonce: $NONCE" -u admin:admin "https://wp_lab.test/wp-json/pulsepress/v1/settings"` (auth via cookies or app password) → confirm `{settings, defaults, choices}` returned with sensible defaults.
...
- [ ] 6.6 Reload single post view; `PulsePressData.iconStyle === 'emoji'` in source view.
- [ ] 6.7 POST `{"allow_guest_reactions": false}`. Submit a reaction as anonymous → 401 `pulsepress_login_required`.

## 7. Docs and final

- [ ] 7.1 Update `docs/hooks-and-filters.md`: add `pulsepress_settings` filter, `pulsepress_settings_default` filter, `pulsepress_settings_saved` action.
