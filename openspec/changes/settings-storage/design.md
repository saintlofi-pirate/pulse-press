...
Up to Session 5, every Moonfarmer Reactions Lead Capture behaviour an admin might want to change lives behind a `apply_filters(...)` call. That's a power-user model. To match the WordPress.org distribution bar and the "best WP-ecosystem plugin" admin direction recently codified in the plan, we need a real settings page.
...
The dominant constraint here is **backward compatibility with the filter surface we already shipped**. A site with `add_filter('moonfarmer_reactions_lead_capture_positive_reactions', ...)` in `functions.php` today must keep working after Session 6 lands. That means: settings are read first, then passed through the same filter, so a filter still wins. The filter is now layered on top of admin-driven defaults rather than replacing them.

## Goals / Non-Goals

**Goals:**

- One canonical settings array under the `moonfarmer_reactions_lead_capture_settings` option.
...
- REST endpoints that the SPA consumes and that Pro can extend through the `moonfarmer_reactions_lead_capture_settings` filter.
...
- No migration of existing `moonfarmer_reactions_lead_capture_delete_on_uninstall` / `moonfarmer_reactions_lead_capture_retention_days` legacy keys in this session. They keep working independently; a later cleanup session removes them once we're confident no one reads them directly.
...
`moonfarmer_reactions_lead_capture_settings` is a single autoloaded option storing the full settings array. Reasons:
...
public function get(): array {
    ...
    $stored   = get_option('moonfarmer_reactions_lead_capture_settings', null);
    $settings = is_array($stored) ? array_merge(self::DEFAULTS, $stored) : self::DEFAULTS;
    $settings = (array) apply_filters('moonfarmer_reactions_lead_capture_settings', $settings);
    $this->cached = $settings;
    return $settings;
}
```

A site with `add_filter('moonfarmer_reactions_lead_capture_settings', fn($s) => ['positive_reactions' => ['love']] + $s)` overrides the admin choice. This preserves filter-only sites' behaviour and gives Pro a clean extension seam.
...
### D6. Settings save fires `moonfarmer_reactions_lead_capture_settings_saved` after the write

```php
public function save(array $partial): array {
    $previous = $this->get();
    $merged   = array_merge($previous, Settings::sanitise($partial));
    update_option('moonfarmer_reactions_lead_capture_settings', $merged + ['_version' => Settings::SCHEMA_VERSION], true);
    $this->cached = $merged;
...
`/wp-json/moonfarmer-reactions-lead-capture/v1/settings` — `GET` returns `{settings, defaults, choices}`. `POST` (treated as upsert) accepts the partial settings to merge. `permission_callback => fn() => current_user_can('manage_options')`.
...
- `Reactions::DEFAULT_POSITIVE` stays as-is. `Settings::get()['positive_reactions']` is passed through `moonfarmer_reactions_lead_capture_positive_reactions` so filter snippets still win.
- `WidgetServiceProvider::enqueueAssets` reads settings and passes `iconStyle`, `themeMode`, `widgetDesign`, `countVisibility`, `countThreshold` into `MoonfarmerReactionsLeadCaptureData`.
...
`add_options_page('Moonfarmer Reactions Lead Capture', 'Moonfarmer Reactions Lead Capture', 'manage_options', 'moonfarmer-reactions-lead-capture', $callback)`. The callback outputs:

```php
<div class="wrap"><div id="moonfarmer-reactions-lead-capture-admin">Loading…</div></div>
```

Session 6b enqueues the admin SPA bundle on this page only and mounts the Preact app into `#moonfarmer-reactions-lead-capture-admin`. The "Loading…" text is the SR-friendly fallback for JS-disabled / failing.

### D10. Backward-compat shim for the two legacy option keys

`Settings::get()['delete_on_uninstall']` falls back to `get_option('moonfarmer_reactions_lead_capture_delete_on_uninstall', '0') === '1'` when the new settings array doesn't have the key yet. Same for `retention_days`. On first save, the new settings array picks up these values and the shim becomes a no-op.
...
No data migration. First read returns DEFAULTS; first save writes the option. Existing `moonfarmer_reactions_lead_capture_delete_on_uninstall` / `moonfarmer_reactions_lead_capture_retention_days` are read through the shim until the new option exists.

Rollback is `git revert` + `delete_option('moonfarmer_reactions_lead_capture_settings')` — the legacy keys are still in place from Session 1.

## Open Questions

- **Q1**: should we send the full settings array down to the front-end widget, or only the bits the widget needs? → **Decided: only the widget-relevant slice**, to keep the widget bundle's `MoonfarmerReactionsLeadCaptureData` small and avoid leaking admin-only fields (consent text version, retention policy) to anonymous visitors.
