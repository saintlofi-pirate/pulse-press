...
- [x] 1.1 Add a stable id constant `CAPTURE_TITLE_ID = 'moonfarmer-reactions-lead-capture-capture-title'` near the existing id constants.
...
- [x] 1.3 Promote `i18n.prompt` rendering to an `<h3 id={CAPTURE_TITLE_ID} class="moonfarmer-reactions-lead-capture-capture-title">` so the dialog's accessible name comes from existing copy without doubling text.
- [x] 1.4 Verify Escape + focus-return still work via the existing keyDown handler (no logic change needed).
- [x] 1.5 Add `.moonfarmer-reactions-lead-capture-capture-title { font-weight: 600; font-size: 1rem; margin: 0 0 0.5rem; }` (or equivalent matching the surrounding visual rhythm) to `resources/widget/styles/widget.css`.
...
- [x] 2.2 Extend `MoonfarmerReactionsLeadCaptureAdminData['i18n']` in `resources/admin/types.ts` with the same shape.
...
## 3. Visible focus on .moonfarmer-reactions-lead-capture-panel

- [x] 3.1 In `resources/admin/styles/admin.css`, replace the `.moonfarmer-reactions-lead-capture-panel { outline: none; }` rule with `.moonfarmer-reactions-lead-capture-panel:focus { outline: none; } .moonfarmer-reactions-lead-capture-panel:focus-visible { outline: 2px solid var(--pp-accent); outline-offset: 4px; border-radius: var(--pp-radius); }`.
