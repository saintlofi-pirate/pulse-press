...
`Moonfarmer\ReactionsLeadCapture\Visibility\VisibilityResolver` SHALL expose `shouldRender(int $postId, string $context): bool` and `mode(int $postId): string` returning one of `'auto'|'on'|'off'`. Every render path (auto-insert, block, shortcode, WidgetMarkup) SHALL consult the resolver before emitting markup.
...
The resolver SHALL read `get_post_meta($postId, '_moonfarmer_reactions_lead_capture_widget_state', true)` first. When the value is `'on'`, the resolver SHALL return `true` regardless of any other setting. When the value is `'off'`, the resolver SHALL return `false` regardless of any other setting (including `hide_on_post_types`).

#### Scenario: Explicit on beats hide list

- **WHEN** `_moonfarmer_reactions_lead_capture_widget_state = 'on'` for post 42 AND the post type is in `hide_on_post_types`
- **THEN** `shouldRender(42, 'auto')` returns `true`

#### Scenario: Explicit off beats auto-insert list

- **WHEN** `_moonfarmer_reactions_lead_capture_widget_state = 'off'` for post 42 AND the post type is in `auto_insert_post_types`
- **THEN** `shouldRender(42, 'auto')` returns `false`

#### Scenario: Explicit off blocks block + shortcode too

- **WHEN** `_moonfarmer_reactions_lead_capture_widget_state = 'off'` and someone places the `moonfarmer-reactions-lead-capture/reactions` block on the post
...
- **WHEN** Pages are in `hide_on_post_types` and a Page contains the `moonfarmer-reactions-lead-capture/reactions` block
- **THEN** the block render returns an empty string

#### Scenario: Hide list catches shortcode

- **WHEN** Pages are in `hide_on_post_types` and a Page contains `[moonfarmer-reactions-lead-capture]`
- **THEN** `do_shortcode('[moonfarmer-reactions-lead-capture]')` returns an empty string
...
### Requirement: moonfarmer_reactions_lead_capture_visibility_mode filter is a last-mile override

The resolver SHALL pass the computed mode through `apply_filters('moonfarmer_reactions_lead_capture_visibility_mode', $mode, $postId, $context)` before applying it. The filter MAY return any of `'auto'|'on'|'off'`; invalid values fall back to the original computed mode.

#### Scenario: Pro overrides for a paid feature

- **WHEN** a Pro plugin registers `add_filter('moonfarmer_reactions_lead_capture_visibility_mode', fn() => 'off', 10, 3)` for unlicensed sites
...
- **WHEN** `_moonfarmer_reactions_lead_capture_widget_state` is unset for post 42
- **THEN** `mode(42)` returns `'auto'`

#### Scenario: Garbage value becomes auto

- **WHEN** `_moonfarmer_reactions_lead_capture_widget_state` is set to `'xyz'` (manually corrupted)
- **THEN** `mode(42)` returns `'auto'`
