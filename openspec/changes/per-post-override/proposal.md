...
Free remains generous: every public post type is a candidate for the widget; per-post overrides are an opt-in nicety, not a gate. Privacy stays first-class — no new data collected; meta keys are namespaced under `_pulsepress_`. Accessibility lives in the post-editor meta box: real radio inputs with a visible legend and helper text, keyboard accessible. Hooks-first: every decision point is filterable.

## What Changes

- Add `PulsePress\Visibility\VisibilityResolver` with `shouldRender(int $postId, string $context): bool` and `mode(int $postId): string` (`'auto'|'on'|'off'`). Becomes the single source of truth for "should the widget appear on this post?". Consulted by:
  - `WidgetServiceProvider::maybeAppendWidget` (auto-insert path)
  - `PulsePress\Blocks\ReactionsBlock::render`
  - `PulsePress\Blocks\Shortcode::render`
  - `PulsePress\Blocks\WidgetMarkup::container` returns empty string when resolver says no
- Add post meta key `_pulsepress_widget_state` (string, one of `'auto'|'on'|'off'`, default missing-means-`'auto'`). Saved via the standard post-save flow + REST `register_post_meta` so block-editor saves work alongside classic editor.
- Add `PulsePress\Admin\WidgetStateMetaBox` registered on every public post type via the `pulsepress_meta_box_post_types` filter (default = `get_post_types(['public' => true])`). Renders a three-radio fieldset (Auto / Always show / Always hide) with helper copy.
...
- **REST API**: `GET /pulsepress/v1/settings` response now includes `choices.post_types`. Post meta `_pulsepress_widget_state` becomes REST-accessible per-post via `register_post_meta`.
- **Database changes**: no schema. New post meta keys.
- **Filters introduced**: `pulsepress_meta_box_post_types` (default `get_post_types(['public' => true])`), `pulsepress_visibility_mode` (allows last-mile override of the resolver's decision).
...
- **Free/Pro boundary**: untouched. Pro can hook `pulsepress_visibility_mode` for license-based behaviour (e.g., per-category gating) without modifying Free.
