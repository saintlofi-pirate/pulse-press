...
- [ ] 1.1 Create `app/Blocks/WidgetMarkup.php` (`final class`). Static method `container(int $postId): string` returning the `<div>` HTML; filters attributes via `moonfarmer_reactions_lead_capture_widget_container_attrs`; serialises attributes in insertion order with empty-string values rendered as bare attributes.
...
- [ ] 2.2 Create `blocks/reactions/render.php` that calls `Moonfarmer\ReactionsLeadCapture\Blocks\ReactionsBlock::render($attributes, $content, $block)`.
...
- [ ] 3.1 Create `app/Providers/BlockServiceProvider.php` extending `ServiceProvider`. In `boot()`, hook `init` to `register_block_type(MOONFARMER_REACTIONS_LEAD_CAPTURE_DIR . 'blocks/reactions')` and `add_shortcode('moonfarmer-reactions-lead-capture', [Shortcode::class, 'render'])`.
- [ ] 3.2 Register `BlockServiceProvider::class` in `app/bootstrap.php` after `WidgetServiceProvider`.

## 4. Update auto-insert

- [ ] 4.1 In `WidgetServiceProvider::maybeAppendWidget`, after the singular + filter checks, add: `if (str_contains($content, 'data-moonfarmer-reactions-lead-capture-widget')) { return $content; }`.
...
- [ ] 5.3 Update `tests/Unit/WidgetServiceProviderTest.php` with a scenario: content containing `data-moonfarmer-reactions-lead-capture-widget` is not appended to.
...
- [ ] 6.2 Open a post in Gutenberg, search for "Moonfarmer Reactions Lead Capture" in the inserter, insert the block. Save. View the post. Confirm one widget renders.
- [ ] 6.3 Open a different post, add `[moonfarmer-reactions-lead-capture]` to the body. View. Confirm one widget renders.
...
- [ ] 6.5 Test `[moonfarmer-reactions-lead-capture post_id="<other-post-id>"]` — confirm the widget renders against the other post's counts.

## 7. Docs and final

- [ ] 7.1 Update `docs/hooks-and-filters.md`: add `moonfarmer_reactions_lead_capture_widget_container_attrs`.
