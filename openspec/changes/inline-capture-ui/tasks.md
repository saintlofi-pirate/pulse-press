...
- [x] 1.2 Create `resources/widget/positive.ts` exporting `isPositive(type: ReactionType | null, data: PulsePressData): boolean`. Returns `false` when `type` is null; otherwise checks membership in `data.positiveReactions`.
- [x] 1.3 Update `resources/widget/storage.ts` with `getCapturedFlag(postId)` / `setCapturedFlag(postId)` and matching dismissed helpers. localStorage keys: `pulsepress:captured:{postId}` and `pulsepress:capture_dismissed:{postId}` (the dismissed flag clears on page load — implement as in-memory only).
...
- [x] 2.1 Create `resources/widget/components/CaptureForm.tsx`. Props: `{ postId: number; reactionType: ReactionType; data: PulsePressData; triggerRef: { current: HTMLButtonElement | null }; onDone: (result: { status: 'inserted'|'already_exists' }) => void; onDismiss: () => void }`.
...
- [x] 8.1 Update `docs/hooks-and-filters.md`: add `pulsepress_positive_reactions` filter (shipped, Session 5).
