...
- Add `resources/widget/positive.ts` exporting `isPositive(type: string, data: MoonfarmerReactionsLeadCaptureData): boolean`. Default positive set is `['love', 'insightful', 'funny']`, surfaced through `MoonfarmerReactionsLeadCaptureData.positiveReactions` (filterable in PHP via `moonfarmer_reactions_lead_capture_positive_reactions`).
...
- Add localStorage flag `moonfarmer-reactions-lead-capture:captured:{postId}` set when the form successfully submits OR returns 409 (already exists). Subsequent positive reactions on the same post do NOT re-show the form.
- Add `resources/widget/api.ts::postCapture(...)` typed wrapper around `POST /moonfarmer-reactions-lead-capture/v1/capture`.
- Update `WidgetServiceProvider::enqueueAssets()` to add `positiveReactions` and an `i18n.capture` payload (prompt, label, placeholder, consent, submitting, thanks, networkError, alreadyCaptured) into `MoonfarmerReactionsLeadCaptureData`.
- Add `moonfarmer_reactions_lead_capture_positive_reactions` PHP filter (default `['love', 'insightful', 'funny']`).
...
- **Free/Pro boundary**: untouched. Pro can hook `moonfarmer_reactions_lead_capture_after_capture` (Session 4) to add ESP sync without modifying Session 5's UI.
