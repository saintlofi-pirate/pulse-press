...
- [ ] 1.1 Create `app/Captures/CaptureExporter.php` (`final class`). Constructor `(\wpdb $wpdb)`. Method `stream(callable $emit, array $options = []): int`. Resolves columns via `moonfarmer_reactions_lead_capture_export_columns` filter from the canonical default map. Fires `moonfarmer_reactions_lead_capture_before_export` inside try/catch for `RestException`. Reads in 500-row chunks ordered by id. Emits header line first; then one CSV-escaped line per row. Returns total rows emitted.
...
- [ ] 2.2 In `CaptureServiceProvider::register()` bind the exporter and the controller. In `boot()` register `GET /moonfarmer-reactions-lead-capture/v1/captures.csv` with `permission_callback => fn() => current_user_can('manage_options')`.
...
- [ ] 3.4 Mount the button at the end of `resources/admin/sections/CaptureSection.tsx` inside a `<div class="moonfarmer-reactions-lead-capture-export-region">` block.
- [ ] 3.5 Update `app/Providers/AdminServiceProvider.php::i18n()` with the new strings.

## 4. Styles

- [ ] 4.1 Extend `resources/admin/styles/admin.css` with `.moonfarmer-reactions-lead-capture-export-region` (card-styled), `.moonfarmer-reactions-lead-capture-export-helper`, `.moonfarmer-reactions-lead-capture-export-status` (transient pill). Reuse existing `.moonfarmer-reactions-lead-capture-submit` for the button.
...
- [ ] 5.1 `tests/Unit/CaptureExporterTest.php`:
  ...
  - `moonfarmer_reactions_lead_capture_before_export` action throwing `RestException` short-circuits.
...
- [ ] 6.2 `curl -k -H "X-WP-Nonce: $(wp eval 'echo wp_create_nonce("wp_rest");')" -b cookie.jar "https://wp_lab.test/wp-json/moonfarmer-reactions-lead-capture/v1/captures.csv" -o /tmp/captures.csv` (logged in cookie jar required).
...
- [ ] 7.1 Update `docs/hooks-and-filters.md`: add `moonfarmer_reactions_lead_capture_export_columns` filter and `moonfarmer_reactions_lead_capture_before_export` action.
