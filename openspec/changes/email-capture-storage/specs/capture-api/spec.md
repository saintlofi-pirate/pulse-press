...
- **THEN** the response is `409` with code `pulsepress_capture_already_exists` and message "We already have your email saved for this post." and no new row is written
...
The controller SHALL reject any submission where the `consent` field is not strictly the boolean `true` (not `1`, not `"true"`, not `"on"`). Rejection returns `422` with code `pulsepress_consent_required` and message "You must agree to the consent statement before submitting your email."

#### Scenario: Consent omitted

- **WHEN** the request body lacks a `consent` field
- **THEN** the response is `422` with code `pulsepress_consent_required` and no row is written

#### Scenario: Consent is string "true"

- **WHEN** the request body has `"consent": "true"`
- **THEN** the response is `422` with code `pulsepress_consent_required` — strict boolean check, no coercion

#### Scenario: Consent is boolean false

- **WHEN** the request body has `"consent": false`
- **THEN** the response is `422` with code `pulsepress_consent_required`

### Requirement: Email is normalised, validated, and filterable

The controller SHALL lowercase and trim the submitted email before validation and storage. The normalised email SHALL pass `is_email()` and SHALL be ≤ 190 characters. Sites MAY register the `pulsepress_capture_email` filter to apply additional transformations (e.g., strip `+tag` aliases) — the filter receives the already-lowercased, trimmed email.
...
- **THEN** the response is `422` with code `pulsepress_invalid_email` and message "Please enter a valid email address."

#### Scenario: Email longer than 190 chars

- **WHEN** the submitted email exceeds 190 characters after normalisation
- **THEN** the response is `422` with code `pulsepress_invalid_email` and no row is written

#### Scenario: Filter strips Gmail-style aliases

- **WHEN** `pulsepress_capture_email` is registered to strip `+tag` and the visitor submits `"reader+vip@gmail.com"`
...
The controller SHALL reject submissions whose `reaction_type` is not in the filtered `Reactions::TYPES` allowlist (422 `pulsepress_invalid_reaction_type`) and whose `source` is not in the filtered `Captures::SOURCES` allowlist (422 `pulsepress_invalid_source`).

#### Scenario: Bad reaction type

- **WHEN** `"reaction_type": "applause"` is submitted
- **THEN** the response is `422` with code `pulsepress_invalid_reaction_type` and message "That reaction type is not available on this site."

#### Scenario: Bad source

- **WHEN** `"source": "telegram"` is submitted
- **THEN** the response is `422` with code `pulsepress_invalid_source` and message "That capture source is not allowed."
...
The controller SHALL compute `ip_hash` and `user_agent_hash` via the same HMAC-SHA256 construction as `UserHash::compute`, but with the salt scope `'pulsepress_capture'` (not `'pulsepress_dedup'`) so the capture hash is not bytewise equal to the reaction hash for the same visitor. Each hash SHALL be exactly 64 lowercase hex characters and stored in the CHAR(64) NULL columns.
...
- **THEN** `wp_next_scheduled('pulsepress_purge_fraud_metadata')` returns a timestamp

#### Scenario: Reactivation does not double-schedule

- **WHEN** the plugin is deactivated and reactivated
- **THEN** `wp_next_scheduled('pulsepress_purge_fraud_metadata')` returns exactly one timestamp; no duplicate events are registered

#### Scenario: Deactivation unschedules

- **WHEN** the plugin is deactivated
- **THEN** `wp_next_scheduled('pulsepress_purge_fraud_metadata')` returns `false`

### Requirement: Action hooks expose extension points around the capture write

The controller SHALL fire `do_action('pulsepress_before_capture', int $postId, string $email, string $reactionType, \WP_REST_Request $request)` before invoking the repository, inside a try/catch for `\PulsePress\Http\RestException`. After a successful insert (and only `'inserted'` — not `'already_exists'`) the controller SHALL fire `do_action('pulsepress_after_capture', int $captureId, int $postId, string $email, string $reactionType, string $consentTextVersion)`.

#### Scenario: Before hook can short-circuit

- **WHEN** a plugin registers `add_action('pulsepress_before_capture', $cb, 10, 4)` whose callback throws `new RestException(new WP_Error('pulsepress_rate_limited', 'Too many requests.', ['status' => 429]))`
- **THEN** the response is `429` with code `pulsepress_rate_limited` and no row is written and the after-hook does not fire

#### Scenario: After hook fires only on insert

- **WHEN** a duplicate submission returns 409 `pulsepress_capture_already_exists`
- **THEN** no `pulsepress_after_capture` action is dispatched for that request

#### Scenario: After hook receives full context

- **WHEN** a fresh capture succeeds
- **THEN** `pulsepress_after_capture` is invoked once with arguments `($captureId, $postId, $email, $reactionType, $consentTextVersion)`

### Requirement: Consent text version is exposed via filter

`Captures::consentTextVersion(): string` SHALL return `apply_filters('pulsepress_consent_text_version', 'v1')`. The returned string SHALL be stored verbatim in the `consent_text_version` column on every new capture. Existing rows SHALL NOT be modified when the filter changes the version — old captures keep their original version.
...
- **WHEN** `add_filter('pulsepress_consent_text_version', fn() => '2026-06-policy')` is registered and a new capture lands
...
- **THEN** `pulsepress_post_not_found`, `pulsepress_invalid_reaction_type`, `pulsepress_invalid_email`, `pulsepress_consent_required`, `pulsepress_invalid_source`, and `pulsepress_capture_already_exists` are all distinct and stable
