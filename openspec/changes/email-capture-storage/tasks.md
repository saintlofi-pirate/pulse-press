...
- [ ] 1.1 Create `app/Captures/Captures.php` with `final class` in namespace `PulsePress\Captures`. Expose `public const SOURCES = ['inline', 'block', 'shortcode']`, `public const DEFAULT_VERSION = 'v1'`, `public const PURGE_DAYS = 30`. Add static `isValidSource(string $source): bool` (filtered via `pulsepress_capture_sources`) and static `consentTextVersion(): string` (filtered via `pulsepress_consent_text_version`).
...
- [ ] 3.2 Method body order: parse `post_id`, `email`, `reaction_type`, `consent`, `source` from JSON params. Defaults `source` to `'inline'` when missing. Lowercase + trim email; apply `pulsepress_capture_email` filter; validate `is_email()` and `strlen <= 190`. Validate `consent === true` (strict). Validate `reaction_type` via `Reactions::isValid`. Validate `source` via `Captures::isValidSource`. Validate post via `postIsPublic` (copy or extract from ReactionController). Compute `ip_hash` + `user_agent_hash` via `UserHash::compute(ip, ua)` with a temporary scope swap — see 3.3.
- [ ] 3.3 The capture hash uses a distinct salt scope (`'pulsepress_capture'`). Add a `UserHash::computeForCapture(string $ip, string $ua): string` helper that calls `hash_hmac('sha256', "$ip|$ua", wp_salt('auth') . 'pulsepress_capture')`. Add `UserHash::fromRequestForCapture(\WP_REST_Request $request): array{ip: string, ua: string, ipHash: string, uaHash: string}` so the controller gets all four values in one call and the salt scope stays encapsulated.
- [ ] 3.4 Compute `consentAt = new DateTimeImmutable('now', new DateTimeZone('UTC'))` and `purgeAt = consentAt->modify('+30 days')`. Build `CaptureInput`. Fire `pulsepress_before_capture` inside try/catch for `RestException`. Call `$repository->store($input)`. On `'already_exists'` return 409. On `'inserted'` fire `pulsepress_after_capture` and return 200 with `{post_id, reaction_type, email, status, capture_id}`.
- [ ] 3.5 All error returns use distinct codes and full-sentence messages: `pulsepress_post_not_found` (404), `pulsepress_invalid_reaction_type` (422), `pulsepress_invalid_email` (422), `pulsepress_consent_required` (422), `pulsepress_invalid_source` (422), `pulsepress_capture_already_exists` (409).
...
- [ ] 6.1 Update `pulsepress.php` activation closure: after `Application::boot()`, schedule `pulsepress_purge_fraud_metadata` daily if `wp_next_scheduled` returns `false`. Use `time() + HOUR_IN_SECONDS` as the first-run timestamp.
- [ ] 6.2 Update `pulsepress.php` deactivation closure: unschedule any existing `pulsepress_purge_fraud_metadata` event. Leave option keys and table data intact.

## 7. Test stubs and tests

- [ ] 7.1 Extend `tests/Stubs/wp_functions.php` namespaced shims for `PulsePress\Captures` and `PulsePress\Http\Controllers`: `is_email`, `sanitize_email`, `wp_unslash`, `sanitize_text_field`, `wp_schedule_event`, `wp_unschedule_event`, `wp_next_scheduled`, `time`, `current_time` if needed.
...
- [ ] 8.1 `wp db query "DELETE FROM wp_pulsepress_captures"` for a clean slate.
...
- [ ] 8.3 POST the same `(email, post_id)` again; confirm 409 + `pulsepress_capture_already_exists`.
- [ ] 8.4 POST with `consent: false`; confirm 422 + `pulsepress_consent_required`.
- [ ] 8.5 POST with `email: "not-an-email"`; confirm 422 + `pulsepress_invalid_email`.
- [ ] 8.6 POST with `reaction_type: "applause"`; confirm 422 + `pulsepress_invalid_reaction_type`.
- [ ] 8.7 POST with `source: "telegram"`; confirm 422 + `pulsepress_invalid_source`.
- [ ] 8.8 POST with mixed-case email; confirm the stored row's `email` is lowercase.
- [ ] 8.9 Manually trigger purge: `wp cron event run pulsepress_purge_fraud_metadata`. On a fresh install it's a no-op (rows are < 30 days old). Insert a row with `fraud_metadata_purge_at` in the past via SQL; rerun the cron; confirm hashes are nulled.

## 9. Docs and final verification

- [ ] 9.1 Update `docs/hooks-and-filters.md`: promote `pulsepress_before_capture` and `pulsepress_after_capture` from "planned" to "shipped"; add new filters `pulsepress_capture_sources`, `pulsepress_consent_text_version`, `pulsepress_capture_email`.
- [ ] 9.2 Run PHP lint across all touched files.
- [ ] 9.3 Run `openspec validate email-capture-storage --strict --no-interactive`.
- [ ] 9.4 Confirm `wp-content/debug.log` has no new PulsePress errors after manual verification.
