...
- Pro can attach ESP sync / double opt-in / webhooks by hooking `pulsepress_after_capture` without modifying Free.
...
- No rate-limiting at the application layer. The `pulsepress_before_capture` hook plus the unique key cover the obvious abuse cases; serious rate-limiting is a follow-up module.
...
Captures are write-only on the front end. There is no public "list my captures" endpoint — that would be a privacy hole. Reads come from the admin (CSV export in Session 10, dashboard in Session 9). So `POST /pulsepress/v1/capture` is the entire surface.
...
The controller does `strtolower(trim($input))` before calling `is_email()`. Some admins want to also strip Gmail-style `+tag` aliases to dedupe more aggressively; the `pulsepress_capture_email` filter lets them. The default is plain lowercase+trim — the most defensible behaviour without surprising anyone.

`VARCHAR(190)` means emails longer than that are rejected with `pulsepress_invalid_email`. RFC 5321's 254-char limit is theoretical; 190 covers every real email I've seen in five years of WordPress logs.

### D4. Duplicate (email, post): return 409 `pulsepress_capture_already_exists`, not replace
...
```php
hash_hmac('sha256', $ip . '|' . $userAgent, wp_salt('auth') . 'pulsepress_capture')
```

The `'pulsepress_capture'` scope (vs. `'pulsepress_dedup'` from Session 2) ensures the same visitor's reaction hash and capture hash *don't match*, so a leaked capture table can't be joined to the reactions table to deanonymise visitors. Defense-in-depth.
...
```sql
UPDATE <prefix>pulsepress_captures
...
`pulsepress_purge_fraud_metadata` is registered as a daily WP-Cron event:

- **Activation**: if `wp_next_scheduled('pulsepress_purge_fraud_metadata') === false`, call `wp_schedule_event(time() + HOUR_IN_SECONDS, 'daily', 'pulsepress_purge_fraud_metadata')`. The +1 hour offset avoids racing the activation hook itself.
- **Deactivation**: `wp_unschedule_event(wp_next_scheduled(...), 'pulsepress_purge_fraud_metadata')`. Data stays in tables; only the cron is removed.
...
```json
{
  "code": "pulsepress_consent_required",
...
```php
do_action('pulsepress_before_capture', $postId, $normalisedEmail, $reactionType, $request);
// store
do_action('pulsepress_after_capture', $captureId, $postId, $normalisedEmail, $reactionType, $consentTextVersion);
```

`before_capture` is called inside a try/catch for `RestException` so a rate-limit module can throw a `WP_Error('pulsepress_rate_limited', '…', ['status' => 429])` and the controller will return it. `after_capture` fires only on `'inserted'` — a 409 duplicate skips it, so ESP sync doesn't double-subscribe.
...
- **Risk**: `is_email()` is permissive (allows some technically-invalid addresses). → Mitigation: we accept WP's definition as canonical. The `pulsepress_capture_email` filter lets stricter sites layer extra validation.
- **Risk**: a Pro `pulsepress_after_capture` handler throws an exception, leaving Free with a stored row but no ESP sync. → Mitigation: the controller does not propagate `after_capture` exceptions — Free's perspective is the store succeeded. Pro handlers should log and retry on their own (Pro will land Action Scheduler retry for ESP sync in its own session).
...
1. Land the change.
2. Hit `POST /wp-json/pulsepress/v1/capture` with `consent: false`. Confirm 422 + `pulsepress_consent_required`.
3. Hit with valid `consent: true`, valid email, valid post, valid reaction. Confirm 200 + a row in `wp_pulsepress_captures` with `consent=1`, `consent_text_version='v1'`, populated `ip_hash` and `user_agent_hash`, `fraud_metadata_purge_at` ≈ now + 30 days.
4. Re-submit the same email on the same post. Confirm 409 + `pulsepress_capture_already_exists` and no new row.
5. Re-submit the same email on a *different* post. Confirm 200 + new row.
6. Run `wp cron event run pulsepress_purge_fraud_metadata` manually. Confirm rows older than 30 days have their hashes nulled and newer rows are untouched. (Will be a no-op on a fresh install — assert via SQL.)
