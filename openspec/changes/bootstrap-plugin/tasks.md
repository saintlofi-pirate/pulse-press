...
- [x] 2.1 Write `composer.json` with `require: { "php": ">=8.1" }`, `require-dev: { "pestphp/pest": "^2.34" }`, PSR-4 autoload `"PulsePress\\\\": "app/"`, files autoload `app/Helpers/functions.php`, `autoload-dev` for `Tests\\`, and a `scripts.test` entry running `pest`.
...
- [x] 3.1 Create `pulsepress.php` with WordPress plugin header (`Plugin Name`, `Description`, `Version: 0.1.0`, `Author`, `Text Domain: pulsepress`, `Requires at least: 6.2`, `Requires PHP: 8.1`).
- [x] 3.2 Define `PULSEPRESS_VERSION`, `PULSEPRESS_FILE`, `PULSEPRESS_DIR`, `PULSEPRESS_URL` constants.
...
- [x] 3.4 Register `register_activation_hook(__FILE__, ...)` that calls `update_option('pulsepress_db_version', '0', false)`.
- [x] 3.5 Register `register_deactivation_hook(__FILE__, ...)` as a no-op stub for now.
- [x] 3.6 On `plugins_loaded` (priority 5), require `vendor/autoload.php` and call `\PulsePress\Core\Application::boot(__FILE__)`.
...
- [x] 4.10 Create `app/Helpers/functions.php` containing only a single `pulsepress_container()` helper that returns the application container.
...
- [x] 6.1 Write `package.json` with name `pulsepress`, devDependencies `vite ^5`, `@preact/preset-vite`, `@playwright/test`, dependencies `preact`. Scripts: `dev: vite`, `build: vite build`, `test:e2e: playwright test`.
...
- [x] 6.3 Create `resources/widget/index.ts` with a single comment placeholder: `// PulsePress widget entry — Session 3 will populate.` and an empty `export {}`.
...
- [x] 7.4 Create `tests/Unit/BootstrapTest.php` with one test: `it('defines PULSEPRESS_VERSION when plugin file loads')` that requires `pulsepress.php` in isolation (mocking `register_activation_hook` and friends as needed) and asserts `defined('PULSEPRESS_VERSION') === true`.
...
- [x] 8.2 Write `readme.txt` WordPress.org stub with `=== PulsePress ===`, `Tags: reactions, email, analytics`, `Requires at least: 6.2`, `Tested up to: 6.6`, `Requires PHP: 8.1`, `Stable tag: 0.1.0`, `License: GPLv2 or later`, and a one-paragraph short description.
...
- [x] 9.2 Run `find app pulsepress.php -name '*.php' -print0 | xargs -0 -n1 php -l` and confirm every file reports "No syntax errors".
