...
- Create `moonfarmer-reactions-lead-capture.php` plugin entry with `MOONFARMER_REACTIONS_LEAD_CAPTURE_VERSION`, `MOONFARMER_REACTIONS_LEAD_CAPTURE_FILE`, `MOONFARMER_REACTIONS_LEAD_CAPTURE_DIR`, `MOONFARMER_REACTIONS_LEAD_CAPTURE_URL` constants and a minimum WP/PHP guard.
- Add `composer.json` with PSR-4 namespace `Moonfarmer\ReactionsLeadCapture\\` → `app/` and a `app/Helpers/functions.php` autoload entry. Require PHP `>=8.1`. Keep Pest as the only dev dependency.
...
- Add activation/deactivation hook stubs in the plugin entry that write a `moonfarmer_reactions_lead_capture_db_version` option placeholder (no tables yet — Session 1 owns schema).
...
- **New files**: `moonfarmer-reactions-lead-capture.php`, `composer.json`, `package.json`, `vite.config.js`, `phpunit.xml`, `.gitignore`, `.distignore`, `readme.txt`, `index.php`, `app/**` skeleton, `resources/widget/index.ts` stub, `tests/Pest.php`, `tests/TestCase.php`, one smoke test.
