...
- [x] 1.1 Create `app/Reactions/Reactions.php` with `final class Reactions` in namespace `Moonfarmer\ReactionsLeadCapture\Reactions`, exposing `public const TYPES = ['love', 'insightful', 'funny', 'sad', 'surprised', 'angry']` and a static `isValid(string $type): bool` that runs `in_array($type, apply_filters('moonfarmer_reactions_lead_capture_reaction_types', self::TYPES), true)`.
- [x] 1.2 Create `app/Reactions/UserHash.php` with `final class UserHash` in namespace `Moonfarmer\ReactionsLeadCapture\Reactions`, exposing static `compute(string $ip, string $userAgent): string` returning the HMAC-SHA256 hex with scope `wp_salt('auth') . 'moonfarmer_reactions_lead_capture_dedup'`, and static `fromRequest(\WP_REST_Request $request): string` that resolves IP through the `moonfarmer_reactions_lead_capture_client_ip` filter and UA through `$_SERVER['HTTP_USER_AGENT']` (sanitised).
...
- [x] 2.2 Add `countsForPost(int $postId): array<string, int>` that first reads `get_transient('moonfarmer_reactions_lead_capture_counts_' . $postId)`; on miss runs `SELECT reaction_type, COUNT(*) AS c FROM <prefix>moonfarmer_reactions_lead_capture_reactions WHERE post_id = %d GROUP BY reaction_type` via `$wpdb->get_results`, casts counts to `int`, and writes the array back via `set_transient` with TTL 300. Public method `lastReadWasCached(): bool` exposes whether the most recent `countsForPost` came from cache.
- [x] 2.3 Add `invalidateCounts(int $postId): void` calling `delete_transient('moonfarmer_reactions_lead_capture_counts_' . $postId)`.
...
- [x] 3.2 Implement `react(\WP_REST_Request $request): \WP_REST_Response|\WP_Error`. Steps: read `post_id`/`reaction_type` from JSON params; validate post is published+public via `get_post_status`/`is_post_publicly_viewable`; validate reaction type via `Reactions::isValid`; compute user hash; fire `do_action('moonfarmer_reactions_lead_capture_before_react', ...)` inside a try/catch; call `$repository->replace(...)`; call `$repository->invalidateCounts($postId)`; fire `do_action('moonfarmer_reactions_lead_capture_after_react', ...)`; return `WP_REST_Response` with `{post_id, reaction_type, status, counts}` where counts come from a fresh `countsForPost` call.
...
- [x] 5.1 Extend `tests/Stubs/wp_functions.php`: add namespaced shims for `wp_salt`, `apply_filters`, `do_action`, `get_transient`, `set_transient`, `delete_transient`, `wp_unslash`, `sanitize_text_field`, `get_post_status`, `is_post_publicly_viewable` for use inside `Moonfarmer\ReactionsLeadCapture\Reactions` and `Moonfarmer\ReactionsLeadCapture\Http\Controllers` namespaces.
...
- [x] 6.2 `tests/Unit/UserHashTest.php`: asserts determinism, IP-and-UA sensitivity, scope-string difference from raw `wp_salt('auth')`, fallback through `moonfarmer_reactions_lead_capture_client_ip`.
...
- [x] 7.1 Activate the plugin (already active from Session 1). `wp db query "DELETE FROM wp_moonfarmer_reactions_lead_capture_reactions"` for a clean slate.
- [x] 7.2 Pick an existing published post id (e.g. `wp post list --post_type=post --post_status=publish --format=ids | head`).
- [x] 7.3 Hit `curl https://wp_lab.test/wp-json/moonfarmer-reactions-lead-capture/v1/counts/{id}` and confirm `{post_id, counts: {}, cached: false}`.
- [x] 7.4 Mint a nonce: `wp eval "echo wp_create_nonce('wp_rest');"` (or use an admin login session).
- [x] 7.5 `curl -X POST -H "X-WP-Nonce: <nonce>" -H "Content-Type: application/json" -d '{"post_id":<id>,"reaction_type":"love"}' https://wp_lab.test/wp-json/moonfarmer-reactions-lead-capture/v1/react`. Confirm `200` with `status: "inserted"`.
...
- [x] 7.9 POST with `reaction_type: "applause"` and confirm `422 moonfarmer_reactions_lead_capture_invalid_reaction_type`.
- [x] 7.10 POST with `post_id: 9999999` and confirm `404 moonfarmer_reactions_lead_capture_post_not_found`.

## 8. Final verification

- [x] 8.1 Run `find app moonfarmer-reactions-lead-capture.php uninstall.php -name '*.php' -print0 | xargs -0 -n1 /opt/homebrew/opt/php@8.3/bin/php -l` and confirm no syntax errors.
- [x] 8.2 Run `openspec validate reaction-rest-contract --strict --no-interactive` and confirm clean.
- [x] 8.3 Confirm the WordPress debug log has no new moonfarmer-reactions-lead-capture errors.
