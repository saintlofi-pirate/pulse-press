...
The plugin SHALL declare exactly six default reaction types — `love`, `insightful`, `funny`, `sad`, `surprised`, `angry` — in a single source-of-truth constant `Moonfarmer\ReactionsLeadCapture\Reactions\Reactions::TYPES`. Every code path that accepts a reaction type SHALL validate it against this list (or the filtered version of it) using strict comparison. No reaction type SHALL ever reach SQL without passing through `Reactions::isValid()`.

#### Scenario: Allowlist exposed as a constant

- **WHEN** a developer reads `Moonfarmer\ReactionsLeadCapture\Reactions\Reactions::TYPES`
...
- **WHEN** a plugin registers `add_filter('moonfarmer_reactions_lead_capture_reaction_types', fn() => ['love', 'celebrate'])` and `Reactions::isValid('celebrate')` is invoked
- **THEN** it returns `true`

### Requirement: User hash uses HMAC-SHA256 with per-site wp_salt

`Moonfarmer\ReactionsLeadCapture\Reactions\UserHash::compute(string $ip, string $userAgent)` SHALL return `hash_hmac('sha256', $ip . '|' . $userAgent, wp_salt('auth') . 'moonfarmer_reactions_lead_capture_dedup')`. The output SHALL be exactly 64 lowercase hexadecimal characters to fit the `CHAR(64)` column declared in Session 1.
...
- **WHEN** a hypothetical caller computes `hash_hmac('sha256', '1.2.3.4|Mozilla/5.0', wp_salt('auth'))` (no `moonfarmer_reactions_lead_capture_dedup` suffix)
...
### Requirement: IP source falls back through moonfarmer_reactions_lead_capture_client_ip filter

`UserHash::fromRequest()` SHALL resolve the client IP through `apply_filters('moonfarmer_reactions_lead_capture_client_ip', $remoteAddr, $request)` where `$remoteAddr` is `$_SERVER['REMOTE_ADDR']`. The user agent SHALL be `sanitize_text_field(wp_unslash($_SERVER['HTTP_USER_AGENT'] ?? ''))`.

#### Scenario: CDN override via filter

- **WHEN** a site behind a CDN registers `add_filter('moonfarmer_reactions_lead_capture_client_ip', fn($ip, $req) => $req->get_header('cf-connecting-ip') ?: $ip, 10, 2)`
...
The plugin SHALL register `POST /wp-json/moonfarmer-reactions-lead-capture/v1/react` accepting a JSON body with `post_id` (positive integer) and `reaction_type` (string in the allowlist). The endpoint SHALL require a valid `X-WP-Nonce` header verified with `wp_verify_nonce(..., 'wp_rest')`. On success, the endpoint SHALL upsert a single row in `<prefix>moonfarmer_reactions_lead_capture_reactions` using the `UNIQUE KEY (post_id, user_hash)` to replace any prior reaction by the same hash on the same post. The response SHALL include `{post_id, reaction_type, status: "inserted"|"updated", counts: {type: int, ...}}` reflecting the post-write state.
...
- **THEN** the response is `422` with code `moonfarmer_reactions_lead_capture_invalid_reaction_type` and no row is written
...
- **THEN** the response is `404` with code `moonfarmer_reactions_lead_capture_post_not_found` and no row is written
...
The plugin SHALL register `GET /wp-json/moonfarmer-reactions-lead-capture/v1/counts/(?P<post_id>\\d+)` as a public endpoint (`permission_callback: __return_true`). It SHALL return `{post_id, counts, cached}` where `counts` is an associative array keyed by reaction type. The endpoint SHALL serve from a `moonfarmer_reactions_lead_capture_counts_{post_id}` transient when present; on miss it SHALL execute a single grouped SELECT against `<prefix>moonfarmer_reactions_lead_capture_reactions` and write the result into the transient with a 300-second TTL before returning.
...
After every successful `POST /react`, the controller SHALL call `delete_transient('moonfarmer_reactions_lead_capture_counts_' . $postId)` before returning. The next read SHALL miss the cache and recompute fresh counts.
...
The controller SHALL fire `do_action('moonfarmer_reactions_lead_capture_before_react', int $postId, string $reactionType, string $userHash, \WP_REST_Request $request)` before calling the repository, and `do_action('moonfarmer_reactions_lead_capture_after_react', int $postId, string $reactionType, string $userHash, string $status)` after a successful upsert and cache invalidation. The `before` hook SHALL run inside a `try`-block that catches `WP_Error`-wrapping exceptions, enabling future rate-limit modules to short-circuit a write by throwing.

#### Scenario: Rate-limiter rejects a write

- **WHEN** a plugin hooks `moonfarmer_reactions_lead_capture_before_react` and throws a `\Moonfarmer\ReactionsLeadCapture\Http\RestException` wrapping a `WP_Error('moonfarmer_reactions_lead_capture_rate_limited', 'too many', ['status' => 429])`
...
- **WHEN** a plugin registers `add_action('moonfarmer_reactions_lead_capture_after_react', $callback, 10, 4)` and a write succeeds
...
`Moonfarmer\ReactionsLeadCapture\Providers\RestServiceProvider` SHALL bind `Reactions`, `UserHash`, `ReactionRepository`, and `ReactionController` as singletons in `register()`, and in `boot()` SHALL register an `rest_api_init` callback that calls `register_rest_route` twice — once for `/react` and once for `/counts/(?P<post_id>\\d+)` — under the `moonfarmer-reactions-lead-capture/v1` namespace. The provider SHALL be appended to `app/bootstrap.php`'s providers array after `DatabaseServiceProvider`.

#### Scenario: Routes registered exactly once per request

- **WHEN** WordPress fires `rest_api_init` during a request
- **THEN** `register_rest_route` is called twice with the two Moonfarmer Reactions Lead Capture route definitions and not invoked again later in the same request

#### Scenario: Provider boots without WP-loaded errors

- **WHEN** the plugin is active on PHP 8.1+ and WP 6.2+
- **THEN** no PHP warnings or notices appear in the WordPress debug log on a normal request that does not hit a Moonfarmer Reactions Lead Capture endpoint
