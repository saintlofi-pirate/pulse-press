...
1. `<prefix>pulsepress_reactions` has a `UNIQUE KEY (post_id, user_hash)` — duplicate reactions for the same `(post, user)` must replace the prior row, not insert a second.
...
- No rate limiting at the application layer in this slice. The plan calls for rate-limiting by post id, reaction type, dedup hash, and IP window; that lands in a later session behind a clean `pulsepress_before_react` filter. The UNIQUE KEY already eliminates the "spam reactions" failure mode; what's left is "spam reaction *changes*" which is a UX nuisance, not a data integrity issue.
- No JS widget. That's Session 3.
- No `pulsepress_captures` writes. That's Session 4.
...
```sql
INSERT INTO <prefix>pulsepress_reactions (post_id, reaction_type, user_hash, created_at, updated_at)
VALUES (...)
...
### D3. User-hash salt: `wp_salt('auth') . 'pulsepress_dedup'`

Three options were on the table:

1. **A plugin-owned option `pulsepress_hash_salt`** seeded on activation with `wp_generate_password(64, false)`. Pros: PulsePress controls rotation. Cons: another option to back up, and rotation is a destructive action with no UI yet.
...
3. **`wp_salt('auth') . 'pulsepress_dedup'`** (chosen). Pros: per-site uniqueness from WP's secrets infrastructure, plus a scope string that domain-separates PulsePress from any other plugin using `wp_salt`. Zero new options to manage. If admins rotate `wp_salt` (rare and intentional), dedup resets — acceptable, since salt rotation already invalidates auth cookies.

The scope string `'pulsepress_dedup'` is a constant that lives next to the hash function. If we ever need to migrate the salt scheme, we'll bump the scope string and document the dedup reset.

### D4. IP source: `REMOTE_ADDR` with a `pulsepress_client_ip` filter

Default to `$_SERVER['REMOTE_ADDR']` because that's the connection's source IP and cannot be spoofed by a header. CDN-behind sites need `X-Forwarded-For` or equivalent; rather than guessing, we expose `apply_filters('pulsepress_client_ip', $remoteAddr, $request)`. CDN integration is a deliberate, documented action by the site admin, not a default.
...
`pulsepress_counts_{post_id}` is set with `set_transient($key, $counts, 300)`. On every successful write, `delete_transient($key)` runs. This:
...
`Reactions::isValid($type)` checks `in_array($type, apply_filters('pulsepress_reaction_types', self::TYPES), true)` — strict comparison, no type juggling. Pro and the future settings page extend the list through the filter. The repository never receives unvalidated types.
...
- 404 `pulsepress_post_not_found` — post id resolves to nothing, or post is non-public.
- 422 `pulsepress_invalid_reaction_type` — type is well-formed but not in the allowlist.

`GET /counts/{post_id}` success (200): `{post_id: int, counts: {type: int}, cached: bool}`.

`GET /counts/{post_id}` errors: 404 `pulsepress_post_not_found` if the post id is unknown.

### D8. Rate limiting deferred, extension points reserved

`do_action('pulsepress_before_react', $postId, $reactionType, $userHash, $request)` fires inside `react()` before the repository write. A future rate-limit module can hook this, inspect arguments, and short-circuit by throwing a `WP_Error`-compatible exception caught by the controller. The reverse hook `do_action('pulsepress_after_react', $postId, $reactionType, $userHash)` fires after a successful write; Pro and aggregation will hook this in Sessions 4 and 8.

### D9. Permission callbacks: minimal, declarative

```php
register_rest_route('pulsepress/v1', '/react', [
    'methods'             => 'POST',
...
register_rest_route('pulsepress/v1', '/counts/(?P<post_id>\\d+)', [
    'methods'             => 'GET',
...
- **Risk**: A site behind a CDN that doesn't override `pulsepress_client_ip` will see every visitor sharing the CDN's IP, so the per-post dedup degrades to "per-CDN-IP per-post per-UA". → Mitigation: documented in `readme.txt` under "Behind a CDN" with the filter recipe; UA still adds entropy.
...
- **Risk**: A site with object-cache disabled and millions of posts could fill `wp_options` with `_transient_pulsepress_counts_*` rows. → Mitigation: 300-second TTL keeps the lifetime short, and WP core's transient garbage collection removes expired rows. If this becomes a problem, a future session can move counts behind `wp_cache_*` directly.
...
1. Land the change.
2. Hit `GET /wp-json/pulsepress/v1/counts/1` against a post with no reactions and confirm `{post_id: 1, counts: {}, cached: false}`.
...
- **Q1**: Should the `pulsepress_after_react` action fire async (queued) or sync? → **Decided sync** for this slice. Async firing is an aggregation concern that lands in Session 8 behind the `QueueScheduler` abstraction.
