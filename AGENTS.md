...
- **Maintainable.** Test the contract, not the implementation. Type everything PHP 8.1 lets us type. `final class` by default; inheritance is opt-in. Reuse existing helpers (`Schema::tableName`, etc.) before writing new ones. Migrations + option keys are append-only.
